...
export default function FeaturesSection({ feature }) {
    return (
        <section
            id="features"
            ...
            <div className="relative w-full h-auto mt-8 sm:mt-12 flex flex-col items-center">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 w-full max-w-5xl px-4 sm:px-0">
                    {feature.map((feat, index) => (
                        <div
                            key={index}
                            className="bg-gray-900 p-4 sm:p-5 rounded-xl shadow-md flex items-center gap-4 min-w-[260px] hover:scale-105 transition duration-300 whitespace-nowrap"
                        >
                            <img
                                src={feat.icon}
                                alt={feat.title}
